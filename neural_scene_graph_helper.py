...
def to8b(x):
    print(x.shape)
    return (255 * np.clip(x.detach().cpu().numpy(), 0, 1)).astype(np.uint8  )
...
def plane_pts(rays, planes, id_planes, near, method="planes"):
    ...
    id_planes = id_planes.to(device)
    plane_bds = plane_bds.to(torch.float32).to(rays_o.device)
    plane_normal = plane_normal.to(torch.float32).to(rays_o.device)
    ...
    d_ray_first_plane = torch.matmul((plane_bds[:, 0] - rays_o).float(), plane_normal[:, None].float())
    ...
    plane_id = start_id.to(device) + id_planes.to(torch.float32)
    if method == "planes":
    ...
    n = torch.matmul(d_plane_pose.float(), plane_normal[..., None].float())
    z = torch.matmul(rays_d.float(), plane_normal[..., None].float())
...
def get_closest_intersections(z_vals_w, intersection_map, N_rays, N_obj):
    ...
    id_z_vals = (
        torch.zeros((N_rays * N_obj)).to(device)
        .to(torch.int32)
        .index_add_(
            0,
            intersection_map[:, 0] * N_obj + intersection_map[:, 1],
            torch.arange(0, z_vals_w.shape[0]).to(torch.int32).to(device),
        )
        .reshape(N_rays, N_obj)
    )
    # Create ray-index array
    id_ray = torch.arange(0, N_rays).long().to(device)
    ...
    id_first_intersect = id_z_vals[id_reduced[:, 0], id_reduced[:, 1]].reshape(-1).long()
...
def combine_z(z_vals_bckg, z_vals_obj_w, intersection_map, N_rays, N_samples, N_obj, N_samples_obj=1):
    ...
    Returns:
        ...
        id_z_vals_obj:
    """
    z_vals_obj_w = z_vals_obj_w.float()
    if z_vals_obj_w is None or z_vals_obj_w.shape == torch.Size([1]):
        ...
        # ipdb.set_trace()
        z_vals_obj_w_sparse = (
            torch.zeros([N_rays * N_obj, N_samples_obj]).float().cuda()
            .index_add_(0, intersection_map.long(), z_vals_obj_w)
        ...
        z_vals_obj_w_sparse = torch.reshape(z_vals_obj_w_sparse, [N_rays, N_samples_obj * N_obj]).to(device)
    ...
    if z_vals_bckg is not None:
        z_vals_bckg = z_vals_bckg.to(device)
        ...
        z_vals, z_vals_indices = torch.sort(torch.cat((z_vals_obj_w_sparse.to(device), z_vals_bckg.to(device)), dim=1), dim=1)
